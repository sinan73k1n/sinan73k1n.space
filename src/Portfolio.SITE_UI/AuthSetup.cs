...
public static class AuthSetup
{
    public static int Run(string[] args)
    {
        ...
        Console.WriteLine("1) ÖNCE yukarıdaki anahtarı Authenticator'a EKLE. Bunu atlarsan,");
        Console.WriteLine("   dosyayı kurduktan sonra telefonun eski kodu üretmeye devam eder");
        Console.WriteLine("   ve tekrar giremezsin.");
        Console.WriteLine();
        Console.WriteLine("2) ⚠️  KURULU BİR SİSTEMDE: hedef dosyanın ÜZERİNE YAZMA, BİRLEŞTİR.");
        Console.WriteLine("   Bu dosyada yalnız 3 kimlik anahtarı var; çalışan kurulumda");
        Console.WriteLine("   /etc/portfolio/auth.env AYRICA şunu taşıyor:");
        Console.WriteLine("     ConnectionStrings__Portfolio   (MSSQL bağlantı dizesi)");
        Console.WriteLine("   Üzerine yazarsan o satır SİLİNİR ve uygulama HATA VERMEDEN JSON");
        Console.WriteLine("   deposuna düşer: ziyaretçi eski anlık görüntüyü görür, admin'den");
        Console.WriteLine("   yaptığın düzenlemeler veritabanına HİÇ gitmez.");
        Console.WriteLine("   Birleştirme aracı:  python3 ~/bin/kimlik-birlestir.py");
        Console.WriteLine();
        Console.WriteLine("   (İLK kurulumsa üzerine yazmak sorunsuz:");
        Console.WriteLine("    sudo install -m 0600 -o $USER -g $USER <dosya> /etc/portfolio/auth.env)");
        Console.WriteLine();
        Console.WriteLine("3) systemd biriminde olmalı:  EnvironmentFile=/etc/portfolio/auth.env");
        Console.WriteLine("4) Servisi yeniden başlat, /admin adresinden giriş yap.");
        Console.WriteLine();
        Console.WriteLine("⛔ Bu env dosyası ASLA git'e girmez (.gitignore: *.env).");
        Console.WriteLine();
        Console.WriteLine("🐛 /etc/portfolio KLASÖRÜ root'a ait: servis kullanıcısı oradaki mevcut dosyaya");
        Console.WriteLine("   yazabilir ama YENİ dosya oluşturamaz → `sed -i` ve `.tmp + rename`");
        Console.WriteLine("   desenleri orada ÇALIŞMAZ (Permission denied).");
        Console.WriteLine();
